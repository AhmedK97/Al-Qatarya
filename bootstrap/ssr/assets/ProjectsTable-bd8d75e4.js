...
import _sfc_main$2 from "./ProjectsForm-b1d0ed9b.js";
import _sfc_main$3 from "./ProjectsUploadMediaForm-8ae78427.js";
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(__props.projects.data, (project) => {
        ...
        _push(`</td><td data-label="Phone">${ssrInterpolate(project.customer_name)}</td><td data-label="Status">${ssrInterpolate(project.employee_name)}</td><td data-label="Services">${ssrInterpolate(project.services)}</td><td data-label="Address">${ssrInterpolate(project.space_area)}</td><td data-label="status">`);
        _push(ssrRenderComponent(_sfc_main$4, {
          color: project.status === "pending" ? "warning" : project.status === "in_progress" ? "info" : project.status === "finished" ? "success" : "danger",
          label: project.status,
          small: ""
        }, null, _parent));
        _push(`</td><td data-label="project_date">${ssrInterpolate(project.project_date)}</td><td data-label="About">${ssrInterpolate(project.address)}</td><td data-label="notes">${ssrInterpolate(project.notes)}</td><td data-label="Created At">${ssrInterpolate(project.created_at)}</td><td data-label="Action" class="before:hidden lg:w-1 whitespace-nowrap">`);
