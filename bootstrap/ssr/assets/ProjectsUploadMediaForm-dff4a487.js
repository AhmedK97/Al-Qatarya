...
const _sfc_main = {
  ...
  setup(__props, { emit }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<form><input type="file" multiple${ssrIncludeBooleanAttr(unref(form).progress) ? " disabled" : ""} accept=".jpg, .jpeg, .png, .mkv, .mp4">`);
      ...
      _push(` ${ssrInterpolate(unref(form).errors["files"])}</form><!--]-->`);
    };
  }
};
