...
const { projects, filters, customers, employees } = defineProps({
    projects: {
        type: Object,
        default: [],
    },
...
const formModalTitle = computed(() => {
    return currentlyEditedProject.value?.id
        ? `تعديل مشروع ${currentlyEditedProject.value?.title}`
        : "اضافة مشروع جديد";
});
...
<template>
    ...
    <CardBoxModal
        ...
        scrollable
        :hasCancel="true"
        v-model="isUploadMediaModalOpen"
        :noExit="upload"
        :title="formUploadMediaModalTitle"
    >
        <ProjectsUploadMediaForm
            :project="currentlyUploadMediaProject"
            @uploading="uploading"
        />
    </CardBoxModal>
        ...
        <tbody>
            ...
            <tr key="filters">
                ...
                <td data-label="Filter Name">
                    <input
                        placeholder="الاسـم"
                        v-model="activeFilters.filteredBy.title"
                ...
                <td data-label="Filter Customer">
                    <input
                        placeholder="العميل"
                        v-model="activeFilters.filteredBy.customer"
                        class="w-full h-8 px-2 py-1 border rounded border-primary-100"
                    />
                </td>
                ...
                <td data-label="Filter Employee">
                    <input
                        placeholder="الموظف"
                        v-model="activeFilters.filteredBy.employee"
                        class="w-full h-8 px-2 py-1 border rounded border-primary-100"
                    />
                </td>
                <td></td>
            ...
            <tr v-for="project in projects.data" :key="project.id">
                ...
                <td data-label="Services">
                    <span v-if="project.services_name.length > 0">
                        <span
                            v-for="service_name in project.services_name"
                            :key="service_name"
                        >
                            {{ service_name }}
                        </span>
                ...
                <td data-label="status">
                    ...
                    <PillTag
                        v-else-if="project.status == 'canceled'"
                        label="ملغى"
    ...
    >
        <BaseLevel>
            <BaseButtons>
                <BaseButton
                    v-for="(page, index) in projects.links"
                    :key="index"
                    :active="page.active"
                    :label="page.label"
